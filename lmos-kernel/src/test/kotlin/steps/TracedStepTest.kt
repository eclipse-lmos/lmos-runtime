...
import org.eclipse.lmos.kernel.observe.DefaultAgentTracer
...
class TracedStepTest {

    private val tracedStep = mockk<TracedStep>()
    ...
    fun test_tracing(){
        ...
        coEvery { tracedStep.execute(input) } returns Output(Status.BREAK, input)
        runBlocking {
            assertEquals(Output(Status.BREAK, input), tracedStep.execute(input))
        }
    }
}